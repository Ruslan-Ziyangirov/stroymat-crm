...
-- Клиенты -------------------------------------------------------------
insert into public.clients (name, type, inn, phone, email, address, source, store_id, discount_percent)
select v.name, v.type::client_type, v.inn, v.phone, v.email, v.address, v.source,
       (select id from public.stores order by code limit 1), v.discount
from (values
  ('ООО «СтройДом»',            'company',    '7701234567', '+7 495 111-11-11', 'info@stroydom.ru',  'Москва, ул. Ленина, 1',    'Сайт',        3.0),
  ('ИП Ковалёв А.С.',           'company',    '7702345678', '+7 495 222-22-22', 'kovalev@mail.ru',   'Москва, ул. Мира, 8',      'Рекомендация',2.0),
  ('ООО «РемонтПро»',           'company',    '7703456789', '+7 495 333-33-33', 'zakaz@remontpro.ru','Москва, Кутузовский, 22',  'Холодный звонок', 5.0),
  ('Петров Иван Сергеевич',     'individual', null,         '+7 916 444-44-44', 'petrov@ya.ru',      'Москва, Химки',            'Витрина',     0.0),
  ('ООО «ГрандСтрой»',          'company',    '7704567890', '+7 495 555-55-55', 'office@grand.ru',   'Подольск, Кирова, 12',     'Выставка',    0.0),
  ('Сидорова Мария Павловна',   'individual', null,         '+7 903 666-66-66', 'sidorova@gmail.com','Москва, Северный',         'Сайт',        0.0),
  ('ООО «АльфаМонтаж»',         'company',    '7705678901', '+7 495 777-77-77', 'alfa@montazh.ru',   'Москва, Варшавское ш., 3', 'Сайт',        4.0),
  ('ИП Николаев Д.В.',          'company',    '7706789012', '+7 495 888-88-88', 'nikolaev@bk.ru',    'Подольск, Садовая, 7',     'Рекомендация',0.0)
) as v(name, type, inn, phone, email, address, source, discount)
where not exists (select 1 from public.clients c where c.name = v.name);

-- Заказы (сделки) за последние 8 месяцев -------------------------------
do $$
declare
  m int;
  i int;
...
begin
  ...
  select id into v_manager from public.profiles order by random() limit 1;
  ...
  for m in reverse 7..0 loop
    for i in 1..(12 + (m * 2) % 9) loop
      ...
      -- Перекос в сторону «Продажа», как раньше был перекос в сторону «Завершён».
      v_stage := (array[
        'won','won','won','won','won',
        'meeting_scheduled','proposal_sent','contacted','new','conditional_rejection'
      ])[1 + floor(random() * 10)::int]::deal_stage;

      insert into public.orders (client_id, manager_id, store_id, stage, created_at, updated_at, comment)
      values (v_client, v_manager, v_store, 'new', v_created, v_created, null)
      ...
      update public.orders set stage = v_stage, stage_changed_at = v_created, created_at = v_created, updated_at = v_created
      where id = v_order;

      -- Сделки, ещё не проданные и не отклонённые, получают открытую задачу —
      -- иначе доска показывала бы активные сделки без следующего шага.
      if v_stage in ('new', 'contacted', 'proposal_sent', 'meeting_scheduled') then
        insert into public.deal_tasks (order_id, assignee_id, type, due_at, comment)
        values (v_order, v_manager, 'call', now() + ((1 + floor(random() * 10)::int) || ' days')::interval,
                'Связаться с клиентом и уточнить решение по заказу');
      end if;
    end loop;
  end loop;
end $$;
